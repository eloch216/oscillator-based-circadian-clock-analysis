...
class light_macro_environment : public SteadyModule
{
   public:
    light_macro_environment(
        ...
        :  // Define basic module properties by passing its name to its parent class
          ...
          direct_irradiance_transmittance_op(get_op(output_parameters, "direct_irradiance_transmittance")),
          diffuse_irradiance_transmittance_op(get_op(output_parameters, "diffuse_irradiance_transmittance"))
    {
    }
   ...
   private:
    ...
    double* direct_irradiance_transmittance_op;
    double* diffuse_irradiance_transmittance_op;
    // Main operation
...
{
    return {
        "direct_irradiance_transmittance",  // dimensionless
        "diffuse_irradiance_transmittance"  // dimensionless
    };
}
...
void light_macro_environment::do_operation() const
{
    ...
    if (cosine_zenith_angle <= 0) {
        // In this case, the sun is at or below the horizon, so there is no
        // direct irradiance
        direct_irradiance_transmittance = 0.0;
        diffuse_irradiance_transmittance = 1.0;
    } else {
        // The sun is above the horizon, so use its zenith angle to calculate
        // transmittance.
        //
        // The direct transmission is calculated using Equation 11.11 from
        // Campbell & Norman (1998), solving for
        //  direct_irradiance_transmittance = S_p / S_p0.
        //
        // The indirect transmission is calculated using Equation 11.13 from
        // Campbell & Norman (1998), solving for
        //  indirect_irradiance_transmittance = S_d / S_p0
        constexpr double atmospheric_pressure_at_sea_level = 101325;  // Pa.
        const double pressure_ratio = atmospheric_pressure /
                                      atmospheric_pressure_at_sea_level;  // dimensionless.
        constexpr double proportion_of_irradiance_scattered = 0.3;        // dimensionless.
        direct_irradiance_transmittance = pow(
            atmospheric_transmittance,
            (pressure_ratio / cosine_zenith_angle));
        diffuse_irradiance_transmittance = proportion_of_irradiance_scattered *
                                           (1 - direct_irradiance_transmittance) *
                                           cosine_zenith_angle;
    }

    // Update the output parameter list
    update(direct_irradiance_transmittance_op, direct_irradiance_transmittance);    // dimensionless.
    update(diffuse_irradiance_transmittance_op, diffuse_irradiance_transmittance);  // dimensionless.
}
